...
PlayerRecord *create_leaf_record(Player *p)
{
	...
	*/
	...
	PlayerRecord *pr = (PlayerRecord *)malloc(sizeof(PlayerRecord));
	...
	pr->game_records[WINS] = 0;
	pr->game_records[LOSSES] = 0;
	...
	int p1_wins, int p2_wins)
{
	...
	*/
	...
	if (p1_wins > p2_wins)
	{
		pr = create_leaf_record(p1->player);
		p1->parent = pr;
		p2->parent = pr;
		pr->left_child = p2;
		pr->right_child = p1;
		pr->game_records[WINS] = p1_wins;
		pr->game_records[LOSSES] = p2_wins;
	}
	else if (p2_wins > p1_wins)
	{
		pr = create_leaf_record(p2->player);
		p1->parent = pr;
		p2->parent = pr;
		pr->left_child = p1;
		pr->right_child = p2;
		pr->game_records[WINS] = p2_wins;
		pr->game_records[LOSSES] = p1_wins;
	}
...
int traverse(char player_id[], PlayerRecord *curr, int depth)
{
	if (strcmp(curr->player->id, player_id) == 0)
	{
		return depth;
	}
	else
	{
		...
		{
			int pass = depth + 1;
			a = traverse(player_id, curr->left_child, pass);
		}

		if (curr->right_child != NULL)
		{
			int pass = depth + 1;
			b = traverse(player_id, curr->right_child, pass);
		}
...
int get_player_rank(char player_id[], PlayerRecord *root)
{
	...
	*/
	...
	int res = traverse(player_id, root, 1);
